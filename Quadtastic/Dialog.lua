...
local function window_finish(data, gui_state, w, h)
    ...
    local x = data.win_x or (w - min_w) / 2
    local y = data.win_y or (h - min_h) / 2
    data.min_w, data.min_h, dx, dy, data.dragging =
        Window.finish(gui_state, x, y, data.dragging)
...
local function show_buttons(gui_state, buttons, options)
    ...
    do
        ...
        for key, button in pairs(buttons) do
            ...
            local button_disabled = options and options.disabled and
                                        options.disabled[key]
            local button_options = {disabled = button_disabled}

            local button_pressed = Button.draw(gui_state, nil, nil, nil, nil,
                                               string.upper(button), nil,
                                               button_options)
            ...
            local key_pressed = type(key) == "string" and
                    ...
                    -- Special case since "return" is a reserved keyword
                                        key == "enter" and
                                        imgui.was_key_pressed(gui_state,
                                                              "return"))
            ...
            if options and options.tooltip and options.tooltip[key] then
                Tooltip.draw(gui_state, options.tooltip[key], nil, nil, nil,
                             nil, {tooltip_threshold = 0})
...
local function show_filelist(gui_state, scrollpane_state, filelist, chosen_file)
    ...
    do
        Frame.start(gui_state, nil, nil, nil, 150)
        do
            scrollpane_state = Scrollpane.start(gui_state, nil, nil, nil, nil,
                                                scrollpane_state)
            do
                Layout.start(gui_state, nil, nil, nil, nil, {noscissor = true})
                for _, file in ipairs(filelist) do
                    ...
                    local clicked = imgui.was_mouse_pressed(gui_state,
                                                            ...
                                                            row_width,
                                                            row_height)
                    local selected = chosen_file and chosen_file.name ==
                                         file.name

                    if selected then
                        love.graphics.setColor(
                            gui_state.style.palette.shades.darkest)
                    elseif hovering then
                        love.graphics.setColor(
                            gui_state.style.palette.shades.dark)
                    end
                    if hovering or selected then
                        love.graphics.rectangle("fill", gui_state.layout.next_x,
                    ...
                    if file.type == "directory" then
                        love.graphics.draw(gui_state.style.stylesheet,
                                           gui_state.style.quads.filebrowser
                                               .directory, x,
                                           gui_state.layout.next_y + 1)
                    ...
                    x = x + 11
                    Text.draw(gui_state, x, nil, nil, row_height, file.name,
                              options)
                    local text_width = Text.min_width(gui_state, file.name)
                    gui_state.layout.adv_x =
                        math.max(row_width, text_width + 11)
                    ...
                    if clicked then chosen_file = file end
                    Layout.next(gui_state, "|")
                end
            end
            Layout.finish(gui_state, "|")
            ...
            scrollpane_state.max_y = math.max(gui_state.layout.adv_y,
...
function Dialog.show_dialog(message, buttons)
    ...
    local function draw(app, data, gui_state, w, h)
        do
            window_start(data, gui_state, w, h)
            do
                Layout.start(gui_state)
                imgui.push_style(gui_state, "font", gui_state.style.small_font)
                Label.draw(gui_state, nil, nil, w / 2, nil, data.message)
                Layout.next(gui_state, "|")
        ...
        window_finish(data, gui_state, w, h)
    end

    assert(coroutine.running(), "This function must be run in a coroutine.")
    local transitions = {
        -- luacheck: no unused args
        respond = function(app, data, response) return response end
    }
...
function Dialog.query(message, input, buttons)
    ...
    local function draw(app, data, gui_state, w, h)
        do
            ...
            do
                ...
                Layout.next(gui_state, "|")
                if committed then app.query.respond(S.buttons.ok) end
            end
            Layout.finish(gui_state, "|")
        end
        window_finish(data, gui_state, w, h)
    ...
    local query_state = State("query", transitions, {
        input = input or "",
        message = message,
        buttons = buttons or {escape = S.buttons.cancel, enter = S.buttons.ok},
        was_drawn = false
    })
    -- Store the draw function in the state
...
local function switch_to(data, new_basepath)
    local function create_filelist()
        ...
        local filenames = {}
        for file in lfs.dir(".") do table.insert(filenames, file) end
...
function Dialog.open_file(basepath)
    -- Draw the dialog
    local function draw(app, data, gui_state, w, h)
        if not data.basepath then data.basepath = "/" end
        ...
        if not data.filelist then
            ...
            if mode then
                if mode == "file" then
                    ...
                    if success then
                        data.chosen_file = {type = "file", name = filename}
                    end
        ...
        do
            ...
            do
                Layout.start(gui_state)
                imgui.push_style(gui_state, "font", gui_state.style.small_font)
                data.editing_basepath, new_basepath =
                    InputField.draw(gui_state, nil, nil, intended_width, nil,
                                    data.editing_basepath or data.basepath)
                Layout.next(gui_state, "|")

                data.chosen_file, data.committed_file, data.scrollpane_state =
                    show_filelist(gui_state, data.scrollpane_state,
                                  data.filelist, data.chosen_file)

                if data.committed_file then
                    if data.committed_file.type == "directory" then
                        new_basepath = data.basepath .. os.pathsep ..
                                           data.committed_file.name
                ...
                local clicked_button = show_buttons(gui_state, data.buttons, {
                    disabled = {
                        enter = data.chosen_file == nil or data.chosen_file.type ==
                            "directory"
                    }
                })
                if clicked_button then
        ...
        if new_basepath then
            ...
            if not success then app.open_file.err(err) end
        end
    end
    ...
    local transitions = {
        ...
        respond = function(app, data, response)
            return response, tostring(Path(os.path(data.basepath)) ..
                                          (data.committed_file and
                                              data.committed_file.name or ""))
        end,

        err = function(app, data, err) Dialog.show_dialog(err) end
    }

    local file_state = State("open_file", transitions, {
        basepath = basepath or "/",
        buttons = {escape = S.buttons.cancel, enter = S.buttons.open}
    })
...
function Dialog.save_file(basepath, default_extension, save_button_label)
    ...
    local function process_chosen_filename(app, data, filename)
        ...
        local filepath = tostring(Path(os.path(data.basepath)) ..
                                      (filename or ""))
        if filetype == "directory" then
            -- try to switch to that directory
            local success, err = switch_to(data, filepath)
            if not success then app.save_file.err(err) end
    ...
    local function draw(app, data, gui_state, w, h)
        ...
        do
            ...
            do
                ...
                do
                    local new_basepath
                    data.editing_basepath, new_basepath = InputField.draw(
                                                              gui_state, nil,
                                                              nil,
                                                              intended_width,
                                                              nil,
                                                              data.editing_basepath or
                                                                  data.basepath)
                    if new_basepath then
                ...
                if last_chosen_file ~= data.chosen_file and
                    data.chosen_file.type == "file" then
                    data.editing_filename = nil
                end

                if data.committed_file and
                    valid_filename_candidate(data.committed_file.name) then
                ...
                data.editing_filename, data.committed_filename =
                    InputField.draw(gui_state, nil, nil, intended_width, nil,
                                    data.editing_filename or data.chosen_file and
                                        data.chosen_file.type == "file" and
                ...
                if valid_filename_candidate(data.editing_filename) then
                    local _, extension =
                        Path.split_extension(data.editing_filename)
                    if default_extension and not extension then
                        tooltip_label = S.dialogs.default_extension(
                ...
                local clicked_button = show_buttons(gui_state, data.buttons, {
                    disabled = {
                        enter = not valid_filename_candidate(
                            data.editing_filename)
                    },
    ...
    local transitions = {
        ...
        cancel = function(app, data) return S.buttons.cancel end,
    ...
    local file_state = State("save_file", transitions, {
        basepath = basepath or "/",
        buttons = {escape = S.buttons.cancel, enter = save_button_label}
    })
...
function Dialog.choose_grid_config(current_grid_x, current_grid_y)
        ...
        do
            ...
            do
                ...
                do
                    ...
                    Layout.next(gui_state, "-")
                    data.x = InputField.draw(gui_state, nil, nil, 40, nil,
                                             tostring(data.x))
                    Layout.next(gui_state, "-")
                    Label.draw(gui_state, nil, nil, nil, nil, "y:")
                    Layout.next(gui_state, "-")
                    data.y = InputField.draw(gui_state, nil, nil, 40, nil,
                                             tostring(data.y))
                    Layout.next(gui_state, "-")
                end
                Layout.finish(gui_state, "-")
                ...
                local is_invalid = type(tonumber(data.x)) ~= "number" or
                                       ...
                                       tonumber(data.x) <= 0 or tonumber(data.y) <=
                                       0
                local clicked_button = show_buttons(gui_state, data.buttons, {
                ...
                if clicked_button then
                    if clicked_button == S.buttons.ok then
                        app.choose_grid_config.ok(tonumber(data.x),
                                                  tonumber(data.y))
                    else
    ...
    local transitions = {
        -- luacheck: no unused args
        ok = function(app, data, x, y)
            return S.buttons.ok, {x = x, y = y}
        end,
    ...
    local dialog_state = State("choose_grid_config", transitions, {
        x = current_grid_x,
        y = current_grid_y,
        buttons = {escape = S.buttons.cancel, enter = S.buttons.ok}
    })
    -- Store the draw function in the state
    dialog_state.draw = draw
    return coroutine.yield(dialog_state)
        ...
        do
            ...
            do
                ...
                local x = gui_state.layout.next_x +
                              (gui_state.layout.max_w - icon_w) / 2
                ...
                Label.draw(gui_state, nil, nil, nil, nil,
                           S.dialogs.about(version_info), options)
                Layout.next(gui_state, "|")
                if edition then
                    edition = common.trim_whitespace(edition)
                    local edition_text =
                        S.editions[edition] or "Unknown edition"
                    Label.draw(gui_state, nil, nil, nil, nil, edition_text,
                               options)
                    Layout.next(gui_state, "|")
                end
    ...
    local function draw(app, data, gui_state, w, h)
        do
            ...
            do
                Layout.start(gui_state)
                Label.draw(gui_state, nil, nil, nil, nil,
                           S.dialogs.acknowledgements)
                Layout.next(gui_state, "|")

                do
                    Frame.start(gui_state, nil, nil, 320, 150)
                    imgui.push_style(gui_state, "font_color",
                                     gui_state.style.palette.shades.brightest)
                    do
                        data.scrollpane_state =
                            Scrollpane.start(gui_state, nil, nil, nil, nil,
                                             data.scrollpane_state)
                        do
                            Layout.start(gui_state, nil, nil, nil, nil,
                                         {noscissor = true})
                            for i, software in ipairs(licenses) do
                                imgui.push_style(gui_state, "font",
                                                 gui_state.style.med_font)
                                Label.draw(gui_state, nil, nil, nil, nil,
                                           software.name)
                                imgui.pop_style(gui_state, "font")
                                Layout.next(gui_state, "|")
                        ...
                        data.scrollpane_state.max_y =
                            math.max(gui_state.layout.adv_y,
                                     gui_state.layout.max_h)
                    end
                    Scrollpane.finish(gui_state, data.scrollpane_state)
            ...
            do
                ...
                elseif state == "got_result" then
                    if result.success then
                        ...
                        Label.draw(gui_state, nil, nil, nil, nil, S.dialogs
                                       .update
                                       .latest(
                                       latest_version or
                        ...
                        Label.draw(gui_state, nil, nil, nil, nil,
                                   S.dialogs.update.err_cannot_fetch_version)
                        can_update = true
                    end
                end

                Layout.next(gui_state, "|")
                local clicked_button = show_buttons(gui_state, {
                    S.buttons.close, S.buttons.download
