...
Scrollpane.finish = function(state, scrollpane_state, content_w, content_h)
  ...
  local has_vertical = (scrollpane_state.is_dragging_vertical or total_content_h > inner_h) and scrollpane_state.has_vertical
  ...
  if has_vertical then
    ...
    -- the content
    local rel_vp_pos =
      (scrollpane_state.y + (scrollpane_state.extra_v_lead or 0)) /
      (total_content_h - inner_h)
    ...
    if state and state.input and state.input.mouse.buttons[1] then
      ...
      elseif state.input.mouse.buttons[1].releases > 1 or
             not state.input.mouse.buttons[1].pressed
      then
  ...
  if has_horizontal then
    ...
    -- the content
    local rel_vp_pos =
      (scrollpane_state.x + (scrollpane_state.extra_h_lead or 0)) /
      (total_content_w - inner_w)
